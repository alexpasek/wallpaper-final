...
export default function City(){
 const images=Array.from({length:6},(_,i)=>`/cities/oakville/${i+1}.webp`);
 const hidden=Array.from({length:120}).map((_,i)=>(<span key={i} className="hidden" aria-hidden="true">Oakville</span>));
 return(<div className="container-x py-10">
  <div className="sr-only" aria-hidden="true">{hidden}</div>
  <nav className="text-sm text-gray-500 mb-6"><Link href="/">Home</Link> / Oakville</nav>
  <h1 className="text-3xl font-bold">Oakville — Wallpaper, Popcorn Ceiling, Drywall & Painting</h1>
  <p className="mt-2 text-gray-700">Trusted contractor for wallpaper removal, dust‑free popcorn ceiling removal, drywall installation & repair, and interior painting in Oakville.</p>
